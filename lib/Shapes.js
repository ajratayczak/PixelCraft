...
function star(p1a, p2a){
	...
	if (p1a.x > p2a.x && p1a.y < p2a.y){
		...
		p1 = new Point(p1a.x, p2a.y);
		p2 = new Point(p2a.x, p1a.y);
	}

	let points = [];

	let dx = Math.abs(p2.x - p1.x) + 1;
	let dy = Math.abs(p2.y - p1.y) + 1;
	...
	let left = new Point(p1.x + 1, p1.y + in1len);
	let arr3 = line(left, in1);
	let right = new Point(p2.x - 1, p1.y + in1len);
	let arr4 = line(in2, right);

	let lpoint = new Point(p1.x, p1.y + in1len + 1);
	let rpoint = new Point(p2.x, p1.y + in1len + 1);
	let in3 = new Point(p1.x + in1len - 1, p1.y + (2 * in1len));
	let in4 = new Point(p2.x - in1len + 1, p1.y + (2 * in1len));
	let arr5 = line(lpoint, in3);
	let arr6 = line(rpoint, in4);

	let lbpoint = new Point(p1.x + 1, p2.y - 1);
	let rbpoint = new Point(p2.x - 1, p2.y -1);
	let arr7 = line(in3, lbpoint);
	let arr8 = line(in4, rbpoint);

	let bpoint = new Point(p1.x + 3, p2.y);
	let bpoint2 = new Point(p2.x - 3, p2.y);
	let bottomMidpoint = new Point(midpointx, p2.y - (Math.floor(dy / 4)));
	let arr9 = line(bpoint, bottomMidpoint);
	let arr10 = line(bpoint2, bottomMidpoint);

	points.push(...arr1, ...arr2, ...arr3, ...arr4, ...arr5, ...arr6, ...arr7, ...arr8, new Point(p1.x + 2, p2.y), new Point(p2.x - 2, p2.y), ...arr9, ...arr10);
